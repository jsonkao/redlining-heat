...
data <- inner_join(
  ...
  rbind(fromJSON('../data/task-exports/temperatures-2020.json'), fromJSON('../data/task-exports/temperatures-2000.json'), fromJSON('../data/task-exports/temperatures-1990.json')),
  by = c('neighborho' = 'id')
) %>% 
...
```{r}
TukeyHSD(aov(temperature ~ holc_grade, data = data %>% filter(year == 1990)))
cor.test(data$temperature, data$prop_tree, method = "pearson")
```
